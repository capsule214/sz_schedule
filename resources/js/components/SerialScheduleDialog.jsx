...
export default function SerialScheduleDialog({ plan, gridMode, initialData, onSave, onClose }) {
  ...
  const isMorderPlan = Number(morderId) > 0;
  ...
  function handleSave() {
    ...
    if (!isMorderPlan && !selectedSerialId) { setError('製番を選択してください'); return; }
    if (!taskId) { setError('工程を選択してください'); return; }
    setError('');
    onSave({
      serialId: isMorderPlan ? -1 : Number(selectedSerialId),
      morderId: isMorderPlan ? Number(morderId) : -1,
